...
def get_temporal_loss() -> DiceCELoss:
    """Match the baseline's combined Dice and BCE-style objective."""
    return DiceCELoss(
        sigmoid=True,
        squared_pred=True,
        lambda_dice=1.0,
...
def evaluate_temporal(
    ...
    for batch in tqdm(loader, desc="temporal eval", leave=False):
        sequences = batch["sequence"].to(device, non_blocking=True)
        masks = batch["mask"].to(device, non_blocking=True)
        logits = model(sequences)
        loss = loss_fn(logits, masks)
...
def save_temporal_predictions(
    ...
    for batch in tqdm(loader, desc="temporal predictions", leave=False):
        sequences = batch["sequence"].to(device, non_blocking=True)
        masks = batch["mask"].to(device, non_blocking=True)
        predictions = (torch.sigmoid(model(sequences)) >= threshold).float()
        ...
        for idx in range(sequences.shape[0]):
            ...
            target = masks[idx, 0].detach().cpu().numpy()
            prediction = predictions[idx, 0].detach().cpu().numpy()
            ...
            axes[3].imshow(overlay_mask(center_uint8, prediction_uint8))
            axes[3].set_title("Prediction overlay")
            for axis in axes:
                axis.axis("off")

            sample_id = str(ids[idx])
            fig.tight_layout()
            fig.savefig(
